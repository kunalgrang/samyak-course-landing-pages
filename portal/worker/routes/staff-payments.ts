...
import { getPaymentLedger, recordEnrolmentReceipt, recordEnrolmentReceiptSchema, reverseEnrolmentReceipt, reverseReceiptSchema } from "../lib/payments-ledger";
import { ADMISSION_STAFF_ROLES, RECEIPT_REVERSAL_ROLES, requireStaffRoles } from "../lib/staff-auth";
...
export function registerStaffPaymentRoutes(app: PortalHono) {
  ...
  app.post("/api/staff/enrolments/:enrolmentId/receipts/:receiptId/reversal", async (c) => {
    const originError = requireSameOrigin(c);
    if (originError) return originError;
    ...
    const result = await reverseEnrolmentReceipt(c, staff, c.req.param("enrolmentId"), c.req.param("receiptId"), parsed.data);
    if (!result.ok) return jsonError(c, { status: result.status as 400, code: result.code, message: result.message, fieldErrors: result.fieldErrors });
