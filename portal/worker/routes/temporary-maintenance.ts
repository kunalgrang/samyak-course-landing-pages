...
const requestSchema = z.discriminatedUnion("mode", [
  ...
  z.object({ mode: z.literal("apply"), confirmation: z.literal(IMPORTED_CONTACT_REPAIR_CONFIRMATION) }),
]);
...
export function registerTemporaryMaintenanceRoutes(app: PortalHono) {
  app.post("/api/staff/maintenance/imported-contact-normalization", async (c) => {
    ...
    const result = await runTemporaryImportedContactNormalizationRepair(c, body.mode, body.mode === "apply" ? staff : undefined);
