...
async function runCli() {
  ...
  if (values.remote && !values.preflight) throw new Error("Remote mode is only permitted for future read-only preflight.");
  if (values.remote && values.preflight) throw new Error("Remote preflight is documented but not implemented in this local Phase 2 CLI.");
